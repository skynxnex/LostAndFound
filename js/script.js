onload = function() {
	...
	var photoId;
	var originalThumbnailHeight;
	var originalThumbnailWidth;
	 ...
	 * toggle the sidebar
	 */
	$("#footertoggler").toggle(
			function() {
				var backgroundY = $("#footertoggler")
						.css("background-position").split("px ")[1];
				$("#footertoggler").css( {
					"bottom" : "190px",
	...
	$("#sidebarToggler").toggle(
			function() {
				var backgroundY = $("#sidebarToggler").css(
						"background-position").split("px ")[1];
				$("#sidebarToggler").css( {
					"right" : "0px",
	...
	function backupLocation() {
		$.getJSON('http://www.geoplugin.net/json.gp?jsoncallback=?', function(
				data) {
			latlng = new google.maps.LatLng(data.geoplugin_latitude,
	...
	function displayItemPage(i) {
		...
		$("<h2/>").text(itemList[i].lost_found + " : " + itemList[i].title)
				.appendTo(title);
		if (itemList[i].item_picture_link != null) {
			function x(i) {
				getPhotoFromFlickr(itemList[i].item_picture_link, function(
						photoURL) {
			...
			$(
					"<img class='thumbnail' src='"
							+ itemList[i].where_picture_link + "'/>").appendTo(
					photo);
		}
		if (itemList[i].description != null) {
			$("<p/>").text("Description: " + itemList[i].description).appendTo(
					text);
		}
		if (itemList[i].datetime != null) {
			$("<p/>").text("Found/lost: " + itemList[i].datetime)
					.appendTo(text);
		} else {
	...
	function getPhotoFromFlickr(photoId, callback) {
		...
		$
				.getJSON(
						'http://api.flickr.com/services/rest/?&method=flickr.people.getPublicPhotos&api_key=' + apiKey + '&user_id=59832845@N07&format=json&jsoncallback=?',
						function(data) {
		...
		for ( var i = indexOfTopItem; i < indexOfTopItem + amount; i++) {
			...
			if (itemList[i].item_picture_link != null) {
				function x(i, tempLi) {
					getPhotoFromFlickr(itemList[i].item_picture_link, function(
							photoURL) {
						var img = $("<img class='thumbnail' src='" + photoURL + "'/>")
								.appendTo(tempLi);
						img.hover(imageHoverIn,imageHoverOut);
						var ratio = img.css("width")/70;
						var width = 70;
						var height = img.css("height")*ratio;
						img.css({
							"height": height,
							"width": width
						})
			...
			$("<div/ class='sidebarTitle " + itemList[i].lost_found + "'>")
					.text(itemList[i].lost_found + ": " + itemList[i].title)
					.appendTo($(tempLi));
			if (itemList[i].description != null) {
				$("<div class='sidebarDescription'/>").text(
						"Description: " + itemList[i].description).appendTo(
						$(tempLi));
			}
			if (itemList[i].datetime != null) {
				$("<div/ class='sidebarTime'>").text(
						"When: " + itemList[i].datetime).appendTo($(tempLi));
			} else {
				$("<div/ class='sidebarTime'>").text(
						"When: " + itemList[i].timestamp).appendTo($(tempLi));
			}
...
//		originalThumbnailWidth = this.width();
//		originalThumbnailHeight = this.height();
		var ratio = $(this).css("width")/260;
		var width = 260;
		var height = $(this).css("height")*ratio;
		
			$(this).stop(function(){
				if(this.css("width") >= 260 || this.css("height") >= 50 ) {
					return true;	
				} else {
					return false;
				}
	...
	function pager() {
		...
		if (currentpage > 1) {
			$("<span/ class='previous pagebutton'>").text("Previous").appendTo(
		...
		if (currentpage < topPage) {
			$("<span/ class='next pagebutton'>").text("Next")
